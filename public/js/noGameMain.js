...
function startGame() {
    ...
    levelDescription = "<ul><li> We want a stable material. Only low energy materials are stable.</li>" +
                "<li> The energy of material is dependent on the patterns of atomic arrangement</li>" + 
                "<li> You will be given a layers of atoms. Click each atom to switch its occupation</li>" +
                "<li> Arrange Li (Green), Ni (Red), Co (Blue) to find the stable atomic arrangement/pattern that minimizes the energy!</li> </ul>";
    targetenergy = 0;
    levelActiveAtoms = fillRange(0, 72);
...
function resetLevel(levelDescription){
    ...
    drawbonds(molecules);
    pattern1_classed = Array.apply(null, new Array(72)).map(Boolean.prototype.valueOf,false);
    pattern2_classed = Array.apply(null, new Array(72)).map(Boolean.prototype.valueOf,false);
    pattern3_classed = Array.apply(null, new Array(72)).map(Boolean.prototype.valueOf,false);
    pattern4_classed = Array.apply(null, new Array(72)).map(Boolean.prototype.valueOf,false);
...
function displayLevelSummary(){
	...
	// $("#levelMaxTemp").html(temperatureeachlevel[level-2]);
...
function updatetim() {  //The game timer
    ...
    if(Number(energyvalue).toFixed(2) == 8.55 && checkforMoleculeNumbers()){
        setTimeout(function(){ 	
            nofomoveseachlevel[0] = noofmoves;
            energyeachlevel[0] = energyvalue;
            ...
            console.log("Total timeTaken", endTime - startTime);
    ...
    // if (energyvalue <= targetenergy && gamelevls!=level /* && checkforMoleculeNumbers() */) {
        ...
        // gameTimer.stop();
        // calculateFinalScore();
        // updatePlayerDetails(playerName, level +1,  0, nofomoveseachlevel, energyeachlevel, targeteateachlevel)
        // levelchanged = true;
    // }
...
function gameover() {
   ...
   /*  $(".gameScreen").addClass("hidden");
    ...
    // updatePlayerDetails(playerName, level,  0, nofomoveseachlevel, energyeachlevel, targeteateachlevel)
}

function updatePlayerDetails(playerName, level,  score, nofomoveseach, energyeach, targeteateach){
...
function addTable() {
   ...
   console.log("nofomoveseachlevel", nofomoveseachlevel);
    ...
    for (var i = initialLevel -1; i < nofomoveseachlevel.length; i++) {
        var leveno = i + 1;
        stock[i] = new Array(leveno, nofomoveseachlevel[i],targeteateachlevel[i], Math.round(energyeachlevel[i] * 1e2) / 1e2);
    }
    ...
    //TABLE ROWS
    for (i = initialLevel; i < stock.length; i++) {
        var tr = document.createElement('TR');
